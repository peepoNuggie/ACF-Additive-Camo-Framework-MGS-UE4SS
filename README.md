...
⚠️ **Significant open problem, still unresolved**: even with a fully correct, uniquely-identified `Camouf_72_asset` (verified via `retoc list`), `forcecamo 0 72` does nothing, and live reflection (`FindAllOf("CamouflageAssetType")`) confirms the asset is **never loaded at all**. A follow-up test deliberately overriding an *existing* known name (`Camouf_12_asset`) with modified content also failed to be picked up — ruling out "it only affects brand-new names." Current leading theories: (1) a hard limitation where runtime asset discovery relies on a frozen `AssetRegistry.bin` from the base game's cook time, not a live scan of mounted mod content, or (2) a gap in the retoc/repak round-trip's handling of a package's IoStore identity for cloned/edited assets specifically (a chunk-ID anomaly was observed during the override test that doesn't fit the simple "PackageName determines chunk ID" model from the earlier bug fix). Unresolved — needs a diagnostic testing whether a completely *unmodified* round-tripped asset can be recognized as an override at all, to isolate "game limitation" from "tooling round-trip bug."
