...
🔄 **New, more promising lead**: `sv_camouflage_C`'s parent class `/Script/CobraUI.CCamouflageMenuState` has a `Mgs3UniformCobraUiKeyMap` (map, likely translating camo IDs to UI display keys — if a new ID isn't in this map, it may never be listed, independent of `DT_CamouflageCollection`/enum registration) and a separate `UniformSortDeltaDataTable` (DataTable, distinct from `DT_CamouflageCollection`, likely governing UI sort order). This may be the actual missing piece for menu visibility. Not yet inspected in detail — next step.
...
❌ Not yet confirmed: whether a newly-registered camo shows up in the in-game TAB menu at all — the `Mgs3UniformCobraUiKeyMap`/`UniformSortDeltaDataTable` lead above is the current best path to answering this
