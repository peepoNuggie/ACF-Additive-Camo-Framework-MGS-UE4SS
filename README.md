...
⚠️ **Significant open problem, still unresolved**: even with a fully correct, uniquely-identified `Camouf_72_asset` (verified via `retoc list`), `forcecamo 0 72` does nothing, and live reflection (`FindAllOf("CamouflageAssetType")`) confirms the asset is **never loaded at all**. A follow-up override test also failed. Then, hooking the `DataAssetHelper:LoadDataAsset` UFunction directly (via UE4SS `RegisterHook`) revealed a bigger correction: **that function is never called at all when switching camos**, for any ID, fresh or cached — meaning our long-standing assumption about which function does the real loading work was wrong. The real native code path that actually loads camo visuals is still unidentified; it's likely a private/internal method that bypasses Unreal's reflection dispatch entirely (invisible to UE4SS's Lua hooking, which only intercepts reflected/Blueprint-callable calls). Further progress likely needs either hooking a different, not-yet-identified function, or static disassembly (Ghidra) of the compiled game code.
